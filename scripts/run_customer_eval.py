#!/usr/bin/env python3
"""Run evaluation for Customer Chatbot."""
...
from evaluation.judges.search_judge import SearchJudge
...
eval_config = settings.customer_eval
...
def create_judges():
    ...
    # Search Judge
    search_judge_cfg = getattr(eval_config, "search_judge", None)
    if search_judge_cfg and getattr(search_judge_cfg, "enabled", False):
        llm_client = LLMClientSelector.create(
            provider="litellm",
            proxy_url=shared.llm.proxy_url,
            api_key=shared.llm.api_key,
            completion_model=getattr(search_judge_cfg, "model", "gpt-4o"),
            temperature=getattr(search_judge_cfg, "temperature", 0.0),
        )
        judges.append(
            SearchJudge(
                llm_client=llm_client,
...
def main():
    """Run evaluation."""
    logger.info("Starting Customer Chatbot Evaluation")
    ...
    print("\n" + "=" * 60)
    print("Customer Chatbot Evaluation Summary")
    print("=" * 60)
