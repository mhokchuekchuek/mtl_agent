...
def build_evaluation_service() -> tuple[EvaluationService, str]:
    ...
    eval_config = config.customer_eval
    ...
    # Search Judge
    search_cfg = getattr(eval_config, "search_judge", None)
    if search_cfg and getattr(search_cfg, "enabled", False):
        prompt_cfg = getattr(search_cfg, "prompt", None)
        llm_client = LLMClientSelector.create(
            provider="litellm",
            proxy_url=shared.llm.proxy_url,
            api_key=shared.llm.api_key,
            completion_model=getattr(search_cfg, "model", "gpt-4o"),
            temperature=getattr(search_cfg, "temperature", 0.0),
            max_tokens=getattr(search_cfg, "max_tokens", 16384),
        )
        judge = JudgeSelector.create(
            provider="search",
            llm_client=llm_client,
    ...
    dataset_path = getattr(eval_config, "dataset_path", "evaluation/datasets/customer")
    results_path = getattr(eval_config, "results_path", "results/customer")
