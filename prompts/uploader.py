...
class PromptUploader:
    ...
    def __init__(self, settings: BaseConfigManager | None = None):
        ...
        Args:
            settings: Application settings. If None, creates new ConfigManager.
        """
        self.settings = settings or ConfigSelector.create(provider="dynaconf")

        # Initialize prompt manager (keys resolved from config via Dynaconf @format)
        self.prompt_manager = PromptManagerSelector.create(
            provider=self.settings.get("prompt_manager.langfuse.provider", "langfuse"),
            public_key=self.settings.get("prompt_manager.langfuse.public_key"),
            secret_key=self.settings.get("prompt_manager.langfuse.secret_key"),
            host=self.settings.get("prompt_manager.langfuse.host"),
        )

        # Get prompts configuration
        self.prompts_dir = Path(
            self.settings.get("prompt_manager.directory", "prompts")
        )
        self.version = self.settings.get("prompt_manager.version", "v1")
        self.label = self.settings.get("prompt_manager.label", "latest")
