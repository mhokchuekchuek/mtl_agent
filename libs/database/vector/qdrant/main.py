...
class VectorStoreClient(BaseVectorStore):
    ...
    def add(
        ...
        try:
            ...
            logger.info(
                f"Added {len(points)} points to collection '{self.collection_name}'"
            )
    ...
    def get(
        ...
        """Retrieve points by filter using scroll.

        Args:
            filter: Metadata filter (e.g., {"product_id": 123})
            with_vectors: Whether to include embedding vectors
            limit: Maximum number of results

        Returns:
            List of results with id, metadata, and optionally vector
        ...
        try:
            ...
            # Scroll to get points with optional vectors
            ...
            logger.info(
                f"Retrieved {len(results)} points "
                f"(filter={filter}, with_vectors={with_vectors})"
            )
            return results
